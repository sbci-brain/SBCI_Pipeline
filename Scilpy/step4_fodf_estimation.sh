...
$SRUN fast -t 1 -n 3 -H 0.1 -I 4 -l 20.0 -g -o t1_brain.nii.gz t1_warped.nii.gz
...
#compute FRF
$SRUN scil_compute_ssst_frf.py dwi_fodf.nii.gz bval_fodf bvec_fodf frf.txt --mask b0_mask_resampled.nii.gz\
        --fa 0.7 --min_fa 0.5 --min_nvox 300 --roi_radius 20

#compute fodf and fodf metrics
mkdir fodf

#scil_compute_fodf.py dwi_resampled.nii.gz bvals_xy bvecs_xy frf.txt --sh_order 8\
# --force_b0_threshold --mask b0_mask_resampled.nii.gz\
# --fodf fodf/fodf.nii.gz --peaks fodf/peaks.nii.gz\
# --peak_indices fodf/peak_indices.nii.gz --processes 2 #this should be 1 

$SRUN scil_compute_fodf.py dwi_fodf.nii.gz bval_fodf bvec_fodf frf.txt --sh_order 8\
 --force_b0_threshold --mask b0_mask_resampled.nii.gz\
 --fodf fodf/fodf.nii.gz --peaks fodf/peaks.nii.gz\
 --peak_indices fodf/peak_indices.nii.gz --processes 1 #this should be 1
...
$SRUN scil_compute_fodf_metrics.py fodf/fodf.nii.gz ${a_threshold}\
