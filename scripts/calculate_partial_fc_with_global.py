...
def _build_args_parser():
    ...
    p.add_argument('--gsl', action='store', metavar='GSL', required=True,
                   type=str, help='Path to the motion nuisance regressor file.')
...
def main():
    ...
    confounders = np.concatenate([np.ones([1, 300]),
                                  ...
                                  np.genfromtxt(args.vcsf, dtype=np.float64).T[0:5, :],
                                  np.genfromtxt(args.gsl).reshape([1, 300])]).T
