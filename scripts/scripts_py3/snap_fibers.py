...
# Python 3 port:
#   - xrange() → range()
#   - np.int → int / np.int64 (removed in NumPy 1.24+)
#   - VTK cell array uses SetCells() API (VTK 9.x)
...
def get_surface_by_id(surfaces, surface_map, surf_id):
    ...
    # set all the triangles — Python 3 / VTK 9.x: use np.int64 and SetCells() API
    vtk_triangles = np.hstack(
        np.c_[np.ones(len(triangles), dtype=np.int64) * 3, triangles])
...
def main():
    ...
    lh_limit = surfaces[0].GetNumberOfCells()
