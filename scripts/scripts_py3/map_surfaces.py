...
# -*- coding: utf-8 -*-
# Python 3 port:
#   - OrderedDict.values() returns a view in Python 3; converted to list where needed
#   - np.savez_compressed with ragged mapping array requires object dtype
...
def main():
    ...
    if args.matlab_output is not None and isfile(args.matlab_output):
        if args.overwrite:
    ...
    # load the low resolution surfaces, the vertices of the high resolution surfaces,
    # and initialise cell locator algorithms
    surfaces, points, locators = initialise_surfaces(
        [args.lh_surface_hi, args.lh_surface_lo, args.rh_surface_hi, args.rh_surface_lo])
    ...
    for surface_id in range(len(surfaces)):
        ...
        for i in range(n):
            ...
            if np.allclose(points[surface_id].GetPoint(i),
                           surfaces[surface_id].GetPoints().GetPoint(index)):
                if offset == 0:
                    leftvertex[index] = i
                else:
                    rightvertex[index] = i
    ...
    # order the mapping to low resolution vertex order
    snapped = OrderedDict(sorted(snapped.items(), key=lambda t: t[0]))

    # Python 3: .values() returns a view; convert to list for indexing and len()
    vertices_list = list(snapped.values())
    n = len(vertices_list)
    ...
    # Python 3 + NumPy 1.24+: ragged arrays require explicit object dtype
    mapping_array = np.empty(len(vertices_list), dtype=object)
    for i, v in enumerate(vertices_list):
        mapping_array[i] = np.array(v, dtype=np.int64)
    ...
    if args.matlab_output is not None:
        ids = [[i+1]*len(x) for i, x in enumerate(vertices_list)]
        mapping_concat = np.array([np.concatenate(vertices_list)+1,
                                   np.concatenate(ids)])
        sbci_map = {'map': mapping_concat, 'shape': shape}
