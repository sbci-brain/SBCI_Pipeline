...
# -*- coding: utf-8 -*-
# Python 3 port:
#   - scipy.stats.mode() returns ModeResult; index with [0] still works
#     but in scipy >= 1.9 keepdims parameter changed behaviour.
#     Use .mode[0] for scalar extraction compatible with all scipy versions.
...
def _mode(arr):
    """scipy.stats.mode wrapper compatible with scipy < 1.9 and >= 1.9."""
    result = stats.mode(arr)
    # scipy < 1.9 returns scalar in .mode[0]; >= 1.9 may return array
    m = result.mode
...
def main():
    ...
    if isfile(args.matlab):
        if args.overwrite:
            logging.info('Overwriting "{0}".'.format(args.matlab))
        else:
    ...
    lh_vertices = np.array([_mode(lh_labels[mapping[i]]) for i in range(shape[1])])
    rh_vertices = np.array([_mode(rh_labels[mapping[i] - shape[4]]) for i in range(shape[1], shape[0])])
