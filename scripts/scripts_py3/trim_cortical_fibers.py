...
def save_vtk_streamlines(filename, streamlines):
    ...
    # VTK 9.x requires VTK_ID_TYPE (int64) for cell connectivity and
    # SetCells() instead of the deprecated GetData().DeepCopy() pattern.
    lines_array = ns.numpy_to_vtk(np.array(lines_array, dtype=np.int64),
                                  deep=True, array_type=vtk.VTK_ID_TYPE)
    vtk_lines = vtk.vtkCellArray()
    vtk_lines.SetCells(nb_lines, lines_array)
    ...
    for i in range(0, len(intersection)-1):
        for j in range(i+1, len(intersection)):
            ...
            # if both ROIs are subcortical regions
                ...
                combined_idx = list(zip(roi_idx_a, [roi_a]*len(roi_idx_a))) \
                                   + list(zip(roi_idx_b, [roi_b]*len(roi_idx_b)))

                combined_idx = sorted(combined_idx, key = lambda x:x[0][0])
                combined_mask = [x[1] for x in combined_idx]
                # np.array() on a ragged list of (array, int) tuples raises
                # ValueError in NumPy 1.24+; use a list comprehension instead.
                keep = np.concatenate([[True], np.diff(combined_mask) != 0])
                combined_idx = [combined_idx[i] for i, k in enumerate(keep) if k]
...
def main():
    ...
    label_data = np.asarray(label_img.dataobj).astype(int)
    voxel_dim = label_img.header['pixdim'][1:4]
