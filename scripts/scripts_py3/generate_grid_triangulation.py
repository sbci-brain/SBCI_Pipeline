...
def main():
    ...
    # Python 3 port: np.float → float
    result = np.zeros((lh_n + rh_n, 5), float)

    for i in range(lh_n + rh_n):
        hemisphere = int(i >= lh_n)
        # Python 3 port: fixed tab indentation (was mixed tab/space)
    ...
    # Python 3 port: np.float → float
    tri_result = np.zeros((triangles[0].shape[0] + triangles[1].shape[0], 5), float)
    ...
    for i in [0, 1]:
        ...
        for j in range(vertices.shape[0]):
            normal = np.cross(vertices[j, 1, :] - vertices[j, 0, :],
