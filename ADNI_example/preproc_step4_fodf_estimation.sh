...
echo "Begin FODF preprocessing: $(date)"
...
scil_compute_ssst_frf.py dwi_fodf.nii.gz fodf.bval fodf.bvec frf.txt \
	--mask b0_mask_resampled.nii.gz \
        --fa 0.7 --min_fa 0.5 --min_nvox 300 --roi_radius 20 -f

# compute fodf and fodf metrics
mkdir fodf

scil_compute_fodf.py dwi_fodf.nii.gz fodf.bval fodf.bvec frf.txt --sh_order 6 \
 --force_b0_threshold --mask b0_mask_resampled.nii.gz \
 --fodf fodf/fodf.nii.gz --peaks fodf/peaks.nii.gz \
 --peak_indices fodf/peak_indices.nii.gz --processes 1 -f
...
echo "Finished FODF preprocessing: $(date)"
